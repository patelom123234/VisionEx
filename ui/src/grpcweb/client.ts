...
const client = new VisionExClient(
  import.meta.env.VITE_API_ENDPOINT,
  {} /* =credentials */,
);

// TODO(#7518): Revisit client methods to throw errors instead of using Promise.
export const translateToMarkdown = async (
  image: Uint8Array,
  	targetLanguage: visionex.grpc.Language,
	model: visionex.grpc.Model,
): Promise<string> => {
  const response = await client.TranslateToMarkdown(
    TranslateToMarkdownRequest.fromObject({ image, targetLanguage, model }),
...
export const translateToImage = async (
	image: Uint8Array,
	targetLanguage: visionex.grpc.Language,
): Promise<string> => {
  const response = await client.TranslateToImage(
    TranslateToImageRequest.fromObject({ image, targetLanguage }),
    await createAuthMetadata(),
  );
...
export const translateTextFromImage = async (
  image: Uint8Array,
  targetLanguage: visionex.grpc.Language,
): Promise<{
  uriImage: string;
  sentences: ReturnType<
    typeof visionex.grpc.Sentence.prototype.toObject
  >[];
}> => {
  const response = await client.TranslateTextFromImage(
    TranslateTextFromImageRequest.fromObject({ image, targetLanguage }),
    await createAuthMetadata(),
  );
...
export const signInToVisionEx = async (idToken: string): Promise<void> => {
  const response = await client.SignIn(
    SignInRequest.fromObject({ googleOpenIdToken: idToken }),
    await createAuthMetadata(),
  );
